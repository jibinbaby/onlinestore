            ...
            <div id="page-inner">
                ...
                <?php
                    if($_SESSION['utype']==='A' || $_SESSION['utype']==='T'){
                        $adm =mysqli_query($con,"SELECT `w_id`, `uname`, `bank_name`, `balance`, `status` FROM `wallet` where uname=11111 and status=1;");
                        $admin_fetch=mysqli_fetch_array($adm);
                        $result =mysqli_query($con,"SELECT * FROM `payment` WHERE status=3 order by date;");
                ?>
